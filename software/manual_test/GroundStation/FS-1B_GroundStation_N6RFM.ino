...
// Code modified by N6RFM to disable password protected commands which appear to be reserved for team control operators.
// Commands for CQ, 73 and QSL by prepared K4KDR  
...
void printControls() {
  ...
  Serial.println(F("c - send CQ"));
  Serial.println(F("r - send message to be retransmitted"));
  Serial.println(F("q - send QSL"));
  Serial.println(F("7 - send 73"));
  Serial.println(F("------------------------------------"));
...
void sendCQ() {
  ...
  // get data to be retransmited
  char optData[32];
  // msg to transmit + some additional underscores for blank space after
  String cq = "cq cq de " + myCall + " " + myGrid;
  // Length (with one extra character for the null terminator)
  int str_len = cq.length() + 1; 
  cq.toCharArray(optData, str_len);
  
  Serial.println();
  Serial.print(F("Sending CQ ... "));
...
void sendQSL() {
  ...
  char optData[32];
  // msg to transmit + some additional underscores for blank space after
  String cq = "QSL de " + myCall + " ";
  // Length (with one extra character for the null terminator)
  int str_len = cq.length() + 1; 
  cq.toCharArray(optData, str_len);
  
  Serial.println();
  Serial.print(F("Sending QSL ... "));
...
void send73() {
  ...
  char optData[32];
  // msg to transmit + some additional underscores for blank space after
  String cq = "73 de " + myCall + " ";
  // Length (with one extra character for the null terminator)
  int str_len = cq.length() + 1; 
  cq.toCharArray(optData, str_len);
  
  Serial.println();
  Serial.print(F("Sending 73 ... "));
...
void setup() {
  Serial.begin(115200);
  Serial.println(F("Simplified FOSSASAT-1B Ground Station Code"));
